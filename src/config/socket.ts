...
const socketConfig = (io: any) => {
  io.on("connection", (socket: any) => {
    ...
    socket.on(
      "joinConversation",
      async (data: { conversationId: string; userId: any }) => {
        const { userId, conversationId } = data;
        console.log(
          `User ${socket.id} and Id ${userId} joined conversation ${conversationId}`
        );
    ...
    socket.on(
      ...
      async (data: { conversationId: string; userId: any }) => {
        ...
        console.log(
          `User ${socket.id} and Id ${userId} left conversation ${conversationId}`
        );
      }
    );
    ...
    socket.on("acceptCall", (data: any) => {
      ...
      console.log(
        `Call accepted by user ${userId} in conversation ${conversationId}`
      );
    });
    ...
    socket.on("declineCall", (data: any) => {
      ...
      console.log(
        `Call declined by user ${userId} in conversation ${conversationId}`
      );
    });
